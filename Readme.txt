...
For each novel data analysis problem, the idea is to create a novel directory containing *at least*
	* `WRK`: working directory for the data analysis problem 
		...
		* `RES`: results of R and Scilab analyses
...
* The format of raw data files are : 
	* from the Cybex machine :
		* '.CTM' (data)
		* '.CTA' (information about .CTM files)
		* '.mbg' (setup)
	* from operators :
		* '.txt' (repartitions of the subjects in groups)
